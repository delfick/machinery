...
@pytest.fixture
async def fake_mocked_later(fake_time: thp.FakeTime) -> AsyncGenerator[thp.MockedCallLater]:
...
class TestTick:
    async def test_keeps_yielding_such_that_yields_are_every_apart(
        self, fake_mocked_later: thp.MockedCallLater
    ) -> None:
        called = []
    ...
    async def test_works_with_0_every(self, fake_mocked_later: thp.MockedCallLater) -> None:
        called = []
    ...
    async def test_stops_if_final_future_stops(
        self, fake_mocked_later: thp.MockedCallLater
    ) -> None:
        called = []
...
class TestTicker:
    ...
    async def test_can_change_the_after_once(self, fake_mocked_later: thp.MockedCallLater) -> None:
        called = []
    ...
    async def test_can_have_a_minimum_wait(self, fake_mocked_later: thp.MockedCallLater) -> None:
        called = []
    ...
    class TestWithAPauser:
        async def test_can_be_paused(self, fake_mocked_later: thp.MockedCallLater) -> None:
            called = []
