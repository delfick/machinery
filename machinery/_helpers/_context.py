...
import types
import weakref
from collections.abc import Callable, Coroutine, Hashable, MutableMapping, Sequence
...
class Tramp:
    ...
    def fut_to_string(
        self, f: asyncio.Future[object] | _protocols.WithRepr, with_name: bool = True
    ) -> str:
    ...
    def silent_reporter(self, res: _protocols.FutureStatus[object]) -> Literal[True] | None:
        """
    ...
    def reporter(self, res: _protocols.FutureStatus[object]) -> Literal[True] | None:
        """
...
class CTX[T_Tramp: _protocols.Tramp = _protocols.Tramp]:
    ...
    _callbacks: MutableMapping[Hashable, _CTXCallback[Any, T_Tramp]] = dataclasses.field(
    ...
    def cancelled(self) -> bool:
        return any(fut.cancelled() for fut in self.futs)
    ...
    def add_done_callback(self, cb: Callable[[_protocols.FutureStatus[object]], None]) -> None:
        def wrapped(_: _protocols.CTX[T_Tramp], res: _protocols.FutureStatus[object]) -> None:
            return cb(res)

        self.add_on_done(wrapped, index=cb)

    def remove_done_callback(self, cb: Callable[[_protocols.FutureStatus[object]], None]) -> int:
        counts: list[int] = []
        for fut in self.futs:
    ...
    async def wait_for_first_future(
        self, *futs: _protocols.WaitByCallback, name: str, include_final: bool = False
    ) -> None:
        """
        ...
        waiter: asyncio.Future[bool] = self.loop.create_future()
        waiter.add_done_callback(self.tramp.silent_reporter)
        self.tramp.set_future_name(waiter, name=f"WAITER_FIRST{{{self.name}}}({name})")
    ...
    async def wait_for_all_futures(
        self, *futs: _protocols.WaitByCallback, name: str, include_final: bool = False
    ) -> None:
        """
    ...
    async def cancel_futures_and_wait(self, *futs: _protocols.WaitByCallback, name: str) -> None:
        """
        ...
        waiting: list[_protocols.WaitByCallback] = []
        ...
        await self.wait_for_all_futures(
