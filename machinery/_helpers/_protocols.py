...
class FutureCTXCallback[T_Ret, T_Tramp: Tramp](Protocol):
    def __hash__(self) -> int: ...
...
class WaitByCallback(Protocol):
    def done(self) -> bool: ...
    def cancel(self) -> bool: ...
    def add_done_callback(self, cb: Callable[[FutureStatus[object]], None]) -> None: ...
    def remove_done_callback(self, cb: Callable[[FutureStatus[object]], None]) -> int: ...
...
class Tramp(Protocol):
    ...
    def fut_to_string(
        self, f: asyncio.Future[object] | WithRepr, with_name: bool = True
    ) -> str: ...

    @property
    def reporter(self) -> Reporter[object]: ...

    @property
    def silent_reporter(self) -> Reporter[object]: ...
...
class CTX[T_Tramp: Tramp](Protocol):
    ...
    async def wait_for_first_future(self, *futs: WaitByCallback, name: str) -> None: ...

    async def wait_for_all_futures(self, *futs: WaitByCallback, name: str) -> None: ...

    async def cancel_futures_and_wait(self, *futs: WaitByCallback, name: str) -> None: ...
...
if TYPE_CHECKING:
    ...
    _WBC: WaitByCallback = cast(asyncio.Future[None], None)
